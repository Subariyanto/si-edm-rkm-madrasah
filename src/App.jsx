...
import PengawasPendamping from './pages/PengawasPendamping';
...
function MadrasahRoutes() {
  return (
    <MadrasahLayout>
      <Routes>
        <Route index element={<MadrasahDashboard />} />
        ...
        <Route path="pengawas" element={<PengawasPendamping />} />
      </Routes>
    </MadrasahLayout>
  );
}
...
function LoginRoute() {
  ...
  if (auth) return <Navigate to={auth.role === 'admin' ? '/admin' : '/madrasah'} replace />;
  return <LoginPage />;
}
...
function MadrasahGuard() {
  const auth = getItem('auth');
  if (auth?.role !== 'madrasah') return <Navigate to="/login" replace />;
  return <MadrasahRoutes />;
}
  ...
  return (
    <HashRouter>
      <Routes>
        ...
        <Route path="/madrasah/*" element={<MadrasahGuard />} />
